...
public class ViewInjectProcessor extends AbstractProcessor {
    ...
    void collectInfo(RoundEnvironment roundEnvironment) {
        ...
        Set<? extends Element> elements = roundEnvironment.getElementsAnnotatedWith(ToSave.class);
        for (Element element : elements) {
            // 获取 BindView 注解的值
//            int viewId = element.getAnnotation(ToSave.class).value();
...
//                // 使用构造函数绑定数据
                MethodSpec.Builder saveBuilder = MethodSpec.methodBuilder("saveData")
                        .addModifiers(Modifier.PUBLIC)
                        .addParameter(ParameterSpec.builder(TypeName.get(typeElement.asType()), "activity").build());
                // 使用构造函数绑定数据
                MethodSpec.Builder recoverBuilder = MethodSpec.methodBuilder("recoverData")
                        .addModifiers(Modifier.PUBLIC)
                        .addParameter(ParameterSpec.builder(TypeName.get(typeElement.asType()), "activity").build()); // 使用构造函数绑定数据
                MethodSpec.Builder clearBuilder = MethodSpec.methodBuilder("clearData")
                        .addModifiers(Modifier.PUBLIC)
                        .addParameter(ParameterSpec.builder(TypeName.get(typeElement.asType()), "activity").build());
                List<VariableInfo> variableList = classMap.get(classFullName);
                for (VariableInfo variableInfo : variableList) {
                    ...
                    saveBuilder.addStatement("$T.setParam(activity.getLocalClassName() + activity.user.getUname()+\"$L\",activity.$L)",
                            SPUtil.class, variableName,variableName);

                    recoverBuilder.beginControlFlow("if($T.getParam(activity.getLocalClassName()+ activity.user.getUname()+\"$L\")!=null)",
                            SPUtil.class, variableName)
                            .addStatement("activity.$L = ($L)$T.getParam(activity.getLocalClassName()+ activity.user.getUname()+\"$L\")",
                                    variableName,variableFullName,SPUtil.class,variableName)
                            .endControlFlow();

                    clearBuilder.addStatement("$T.setParam(activity.getLocalClassName() + activity.user.getUname()+\"$L\",\"\")",
                            SPUtil.class,variableName);
                ...
                TypeSpec typeSpec = TypeSpec.classBuilder(typeElement.getSimpleName() + "$$ViewInjector")
                        .addModifiers(Modifier.PUBLIC)
                        .addMethod(saveBuilder.build())
                        .addMethod(recoverBuilder.build())
                        .addMethod(clearBuilder.build())
                        .build();
